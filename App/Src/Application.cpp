...
void Application::run()
{
    while (1) {
        ...
        processAdcTask();
        processSensorTask();
        processHealthTask(true);
    }
}

void Application::processModbusTask()
{
    modbus_.process();
}

void Application::processAdcTask()
{
    adc_.process();
}

void Application::processSensorTask()
{
    const uint32_t nowMs = GetTick();
    sensorService_.process(nowMs);
}

void Application::processHealthTask(bool allRtosTasksHealthy)
{
    errors_.process();
    ...
    if (allRtosTasksHealthy && !errors_.fatalFault()) {
        watchdog_.kick();
    }
}
